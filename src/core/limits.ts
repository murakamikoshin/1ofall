...
export const RUN = {
  ...
  /**
   * 前の区画で席にいたのに一言も書かなかった人の、抽選の重み。
   *
   * 配信で1000人が見ていると発言枠は8。当たった人が繋がったまま黙ると、
   * その席は区画のあいだ（4〜5部屋）空席のまま残る。挑戦者の聞く声が減り、
   * 書きたい人は座れない。線が切れた人の席は AI が引き取るが、
   * **居るのに黙っている人**は素通りだった。
   *
   * 0 にはしない。確定で落とすと、打つのが遅い人・その部屋だけ迷った人が
   * 二度と座れなくなる。薄く引いて、次の区画で書けばすぐ戻る。
   */
  quietSlotWeight: 0.15,
  /**
   * 疑いの札が付いた相手を、嘘つきが撃つ重みの倍率。
   *
   * 札は覚え書きから**公開の一手**になった。置くと相手に伝わるので、
   * 嘘つきは札の付いた者へ寄って埋めにかかる。
   *
   * 入れる前に「撃たれている者ほど本当のことを言っている」という手掛かりが
   * 壊れないかだけ測った（`tools/doubt-probe.mjs`）。倍率 2.5 と 6 で、
   * 二人以上から撃たれた者が正解を口にしていた率は
   * 崖っぷち 89.4% → 87.3%、通常 49.5% → 47.6%。向きも強さも保つ。
   *
   * 倍率が効くのは**撃つ理由が既にある相手のあいだの選り好み**だけなので、
   * 当たった札は何も呼ばず、外した札（正直に正解を言った人に置いた札）が
   * その人を集中砲火に晒す。読み違いの代金がここで出る。
   */
  doubtPileOn: 2.5,
} as const;
 ...
 *
 * 実測（命6・4区画×6部屋・発言枠4）
 *   黙らせるを使わない  1部屋あたり生存 45.1%　踏破 0.0%
 *   黙らせるを使う      1部屋あたり生存 71.2%　踏破 19.5%　1周 19部屋
 *
 * 発言枠を5から4に落としたのが効いた（嘘つき4人→3人）。
 *
 * **区画は5部屋。** 6部屋だったが、32回目に物差しの配役の写し違いを直したら
 * （正直者が一人でない区画が58%あった）1部屋あたり生存が 69% → 78% に上がり、
 * 1周が16.4分で尺の枠（10〜15分）を割った。5部屋にして14.9分。
 * このモードの説明にもともと「5部屋ごとに顔ぶれを入れ替える」と書いてあった。
 */
