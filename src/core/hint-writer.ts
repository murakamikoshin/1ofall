 ...
 * 口ぶりの癖は、**知識の形に縛られている。**
 *
 * 27回目に顔ぶれを周をまたいで固定したので、口ぶりで人が分かるほうがいい
 * （「いつも言い切る人が迷った部屋」が読みどころになる）。
 * そこで assertive / narrows を四つの型（言い切る・迷う・二つに絞る・中庸）に
 * 割ってみたが、**効かなかった。**
 *
 *   人ごとの一番使う族の割合   52% → 56%
 *   口ぶりで話者を当てた率     32% → 31%
 *   同じ型が一部屋で重なった   3回 → **4回**（変種が減って衝突が増えた）
 *
 * 言える形は知識で決まる（二択は候補が二つあるときだけ、警告は耳打ちのときだけ）。
 * 人の癖で押し切れるのはその上の薄い層しかないので、寄せても族は動かず、
 * 代わりに同じ型が並ぶ。連続値のまま（人ごとに固定）に戻した。
 * 人柄は**裏切り方**のほうに出ている（`liarBias` / `liarHonestyAt`。
 * 「裏切2/5」として画面に出していて、実測で +2.2pt の手掛かりになっている）。
 */
