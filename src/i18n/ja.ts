...
export const ja = {
  ...
  briefing: {
    ...
    rules: [
      ...
      // 25〜26回目に足した札の規則。挑戦者の一番強い手なのに、
      // 手引きに一行も無かった（言い回しの注記だけ）
      '疑いの札を置くと、相手に伝わる。置かれた者は次の一言で扉ひとつを言い切るしかない。',
      '札は代金を取る。言い切らせれば記録は早く固まるが、その人の迷いは読めなくなり、嘘つきは札の付いた者へ寄る。',
      // 27回目に顔ぶれを固定した。名前の横の「裏切2/5」が何なのか言っておく
      '顔ぶれは周をまたいで同じ。よく裏切る常連は、名前の横に「裏切2/5」として残る。',
      // 実測：札を頭から真に受けると損をする（区画の序盤は嘘つきが正直に喋る）
      'ただし区画の序盤は、よく裏切る者ほど本当のことを言う。札が効くのは奥へ行ってから。',
    ],
    modes: {
      standard: [
      ...
      party: [
        ...
        // 二人で押す規則。人数が要る＝合意が要ることを先に言う
        '疑いの札は二人揃って効く。二人から付いた者は、次の一言で扉ひとつを言い切るしかない。',
        '押しすぎると損をする。迷いは「二つに絞れている」証なので、言い切らせるほど読むものが減る。',
      ],
    },
  },
  ...
  challenger: {
    ...
    /*
     * 常連の裏切り歴。いまの役ではないが、飾りでもない
     * （嘘つきの引きは人ごとに重みが違う＝よく裏切る常連は本当によく裏切る）。
     */
    pastLiar: (liar: number, of: number) => `裏切${liar}/${of}`,
    pastLiarHint: 'これまで一緒に越えた区画のうち、嘘つきだった回数。引きの癖は人ごとに違う（いまの役ではない）',
    /*
     * 全員挑戦者だけの言い回し。
     *
    ...
    /*
     * 全員挑戦者の札。公開で、みなが置ける。
     * 二人で押せる（一人で押せると裏切り者が正直者を黙らせる道具になる）。
     */
    doubtHintParty: '二人で置けば、その人は言い切るしかなくなる',
    // 押されている者の行に出す（誰が置いたかは出さない）
    pressedMark: '押されている',
    pressedYou: 'あなたは押されている。扉ひとつを言い切れ',
