...
check('他人の知識は混ざらない',
  JSON.stringify(openOf(a1)?.knowledge) !== JSON.stringify(openOf(a2)?.knowledge)
  || openOf(a1)?.knowledge?.kind === openOf(a2)?.knowledge?.kind && openOf(a1).knowledge.kind === 'honest');
...
// 暴言はサーバーで止まる
const speaker = [a1, a2].find((c) => openOf(c)?.isSpeaker) ?? a1;
